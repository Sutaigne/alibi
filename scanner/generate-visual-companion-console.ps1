...
# ============================================================================
# KEYWORD DATABASE (console-mode: mirrors console-rig-audit.ps1)
# ============================================================================
...
$CheatBrands_COD = @(
    ...
    'rut.gg','old.rut.gg','rutforcod','ruavt','rut and ruavt','rutandruavt',
    'rut launcher','rut v3 launcher','rut v4 launcher','rut v3.exe','rut v4.exe',
    'rutv3.exe','rutv4.exe','rivera unlock','riveras unlock','rivera unlock tool',
    'riveras rut','rut unlocker','rut unlock tool','rut uav','rut-uav'
)
$Spoofer_Brands = @(
...
$VisionAimbots = @(
    'aimmmo','aim mmo','aim_mmo','aimsync','aim sync','aim_sync','aimflux','aim flux','aim_flux',
    'norecoilz','no recoil z','no_recoil_z','divisionx','division x','division_x',
    'predator aim','predatoraim','predator-aim','looplus','loop plus','loop+',
    'apox aim','apoxaim','apox-aim','aimkey','aim key','aimx','aim-x','aim_x',
    'kernaim','kernel aim','kernel-aim','colorbot','color bot','color-bot',
    'pixelbot','pixel bot','pixel-bot','ml aim','ml-aim','machine learning aim',
    'ai aimbot','ai-aimbot','ai_aimbot','vision aimbot','vision-aimbot',
...
$Keywords_High = $CheatBrands_COD + $Spoofer_Brands + $CheatFeature_Names + $InputDevices + $DMA_Indicators + $VisionAimbots
$Keywords_Medium = $DMA_DualUse + $DualUse_Tools + $HidEmulators + $CaptureCardSoftware
...
foreach ($f in $findings) { $findingsByScore[$f.Severity]++ }
...
# Findings table
$findingsRows = ''
$cheatFindings = @($findings | Where-Object { $_.Severity -in @('HIGH','MEDIUM') } | Sort-Object @{E={ switch ($_.Severity) { 'HIGH' {1} 'MEDIUM' {2} } }}, Category)
...
if (-not $findingsRows) {
    $findingsRows = "<tr><td colspan='4' class='empty'>No HIGH or MEDIUM findings.</td></tr>"
}
